/* -*- mode: c++ -*-
 * kaleidoscope::device::keyboardio::Raise -- Kaleidoscope device plugin for Dygma Raise
...
namespace kaleidoscope
{
   ...
   */
      ...
      void RaiseLEDDriver::setCrgbAt(uint8_t i, cRGB crgb)
      {
        ...
        if (i >= RaiseLEDDriverProps::led_count)
          return;
      ...
      {
        for (byte row = 0; row < RaiseKeyScannerProps::matrix_rows; row++)
        {
          for (byte col = 0; col < RaiseKeyScannerProps::left_columns; col++)
          {
            uint8_t keynum = (row * RaiseKeyScannerProps::left_columns) + col;
            uint8_t keyState;
            ...
            if (keyState)
              ThisType::handleKeyswitchEvent(
                  Key_NoKey, KeyAddr(row, (RaiseKeyScannerProps::matrix_columns - 1) - col),
                  keyState);
          }
        }
      }
      ...
      bool RaiseKeyScanner::isKeyswitchPressed(KeyAddr key_addr)
      {
        ...
        if (col >= RaiseKeyScannerProps::left_columns)
        {
          return (bitRead(rightHandState.rows[row],
                          (RaiseKeyScannerProps::matrix_columns - 1) - col) != 0);
        }
        else
        {
