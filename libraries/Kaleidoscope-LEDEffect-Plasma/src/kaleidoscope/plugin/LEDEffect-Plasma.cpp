...
void LEDPlasmaEffect::update(void) {
  ...
  for( row=0; row< ROWS; row++ ) {
    ...
    for( col=0; col< COLS; col++ ) {
      ...
      ::LEDControl.setCrgbAt(KeyAddr(row, col), plasma);
...
void LEDPlasmaWaveEffect::update(void) {
  ...
  for (auto led_index : Runtime.device().LEDs().all()) {
    uint16_t key_hue = plasma_hue + 16 * (led_index.offset() / 4);
    if (key_hue >= 255)          {
      key_hue -= 255;
    }
    cRGB plasma = hsvToRgb(key_hue, plasma_saturation, plasma_value);
    ::LEDControl.setCrgbAt(led_index.offset(), plasma);
  }
...
kaleidoscope::plugin::LEDPlasmaEffect LEDPlasmaEffect;
kaleidoscope::plugin::LEDPlasmaWaveEffect LEDPlasmaWaveEffect;
